...
const _updateCliente = (id, cliente) => {
  const clientes = _getClientes();
  const data = clientes.map((_cliente) =>
    _cliente.id === id ? { id, ...cliente, atualizadoEm: getDate() } : _cliente
  );
  _save("@aplicacao:clientes", data);
...
export const addCliente = (cliente) => ({
  type: ADD_CLIENTE,
  cliente: _addCliente(prepararCliente(cliente)),
});

export const updateCliente = (id, cliente) => ({
  type: UPDATE_CLIENTE,
  cliente: _updateCliente(id, cliente),
});

export const removeCliente = (id) => ({
  type: REMOVE_CLIENTE,
  id: _removeCliente(id),
});

// ACTIONS DE APOIO

export const setClienteParaAlterar = (cliente) => ({
  type: SET_CLIENTE,
  cliente,
});
